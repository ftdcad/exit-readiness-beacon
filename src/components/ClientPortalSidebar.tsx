...
const weeks = [{
  ...
  icon: Calculator,
  modules: [{
    name: 'Executive Discovery Interview',
  ...
  modules: [{
    ...
    path: '/portal/week-4/dd-checklist'
  }, {
    name: 'LOI Review',
    path: '/portal/week-4/loi-review'
  }, {
    name: 'Final Report',
    path: '/portal/week-4/final-report'
  }]
}];
export function ClientPortalSidebar() {
  ...
  const getNavClasses = (path: string, isLocked: boolean = false) => {
    ...
    return isActive(path) ? "bg-accent/10 text-accent font-medium border-r-2 border-accent" : "hover:bg-muted/50 text-foreground";
  };
  return <Sidebar className={collapsed ? "w-14" : "w-64"} collapsible="icon">
      <SidebarContent className="bg-background">
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            {!collapsed && <div className="px-3 py-2 text-xs text-muted-foreground">
        ...
        return <SidebarGroup key={week.number}>
              <SidebarGroupLabel className="flex items-center justify-between">
                ...
                {!collapsed && <div className="flex items-center gap-1">
                    {weekProgress && <Badge variant="outline" className="text-xs">
                        {weekProgress.completedModules}/{weekProgress.totalModules}
                      </Badge>}
                    {weekUnlocked && <Badge variant="secondary" className="text-xs">
                        Active
                      </Badge>}
                  </div>}
              </SidebarGroupLabel>
              ...
              {!collapsed && <SidebarGroupContent>
                ...
                return <SidebarMenuItem key={module.name}>
                          <SidebarMenuButton asChild>
                            {weekUnlocked ? <NavLink to={module.path} className={getNavClasses(module.path)}>
                                <span className="text-sm">{module.name}</span>
                                {moduleCompleted ? <CheckCircle className="h-4 w-4 ml-auto text-primary" /> : isActive(module.path) ? <div className="h-2 w-2 rounded-full bg-primary ml-auto" /> : null}
                              </NavLink> : <div className={getNavClasses(module.path, true)}>
                                <span className="text-sm">{module.name}</span>
                  ...
                  {!collapsed && !weekUnlocked && week.number > 1 && <div className="px-3 py-2 text-xs text-muted-foreground">
                      <div className="flex items-center justify-between mb-1">
                        <span>Recommended Path: Week {week.number - 1} first</span>
                      </div>
                      {weekProgress && <div className="w-full bg-muted rounded-full h-1">
                          <div className="bg-primary h-1 rounded-full transition-all" style={{
